...
export default function Home() {
  ...
  async function updateTicket(ticketId: string, endpoint: string, body: object) {
  ...
  function sendToQc(ticket: Ticket) {
    const note = window.prompt("Nota para el supervisor (opcional):", ticket.artist_note ?? "");
    if (note !== null) void updateTicket(ticket.id, "work", { status: "ready_for_qc", artist_note: note.trim() || null });
  }
  function qualityReview(ticket: Ticket, decision: "approve" | "return_for_rework") {
    const feedback = window.prompt(decision === "approve" ? "Comentario de aprobación (opcional):" : "¿Qué debe corregir el artista?", ticket.supervisor_feedback ?? "");
    if (feedback !== null) void updateTicket(ticket.id, "quality-review", { decision, supervisor_feedback: feedback.trim() || null });
  }
