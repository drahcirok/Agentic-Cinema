      ...
      void (async () => {
        ...
        try {
          ...
          if ((cause as Error).name !== "AbortError") {
            setResults([]);
            setSearchError("No pudimos consultar el directorio. Inténtalo otra vez.");
          }
        } finally {
          setLoading(false);
        }
  ...
  if (selected) {
    return (
      <div className="group flex items-center gap-3 rounded-2xl border border-cyan-300/25 bg-cyan-400/[.07] p-3 shadow-[inset_0_1px_rgba(255,255,255,.03)]">
        ...
        {disabled ? (
          <span className="flex items-center gap-1 rounded-full bg-emerald-400/10 px-2 py-1 text-[10px] font-semibold text-emerald-300"><Check className="h-3 w-3" /> Identidad verificada</span>
        ) : (
          <button type="button" onClick={() => { onSelectedChange(null); setQuery(""); }} className="rounded-full p-2 text-slate-500 transition hover:bg-white/5 hover:text-white" aria-label="Cambiar usuario"><X className="h-4 w-4" /></button>
        )}
      </div>
    );
  }
  ...
  return (
    <div className="relative">
      <div className="workspace-input flex items-center gap-3 focus-within:border-cyan-300/70 focus-within:shadow-[0_0_0_3px_rgba(34,211,238,.06)]">
        ...
        <input role="combobox" aria-label="Buscar integrante" aria-autocomplete="list" aria-expanded={results.length > 0 || Boolean(searchError) || searched} aria-controls="member-search-results" value={query} onChange={(event) => { setQuery(event.target.value); setResults([]); setSearched(false); setSearchError(null); }} placeholder="Busca por @usuario o pega su UID" className="min-w-0 flex-1 bg-transparent text-sm text-white outline-none placeholder:text-slate-500" autoComplete="off" />
      </div>
      {query.trim().length > 0 && query.trim().length < 2 && <p className="mt-2 text-[11px] text-slate-500">Escribe al menos 2 caracteres.</p>}
        ...
        <div id="member-search-results" role="listbox" className="absolute left-0 right-0 top-[calc(100%+.5rem)] z-30 overflow-hidden rounded-2xl border border-slate-700/80 bg-[#0d1828]/98 p-2 shadow-[0_22px_60px_rgba(0,0,0,.5)] backdrop-blur-xl landing-rise">
          {searchError ? (
            <div className="px-4 py-5 text-center">
              ...
              <p className="mt-1 text-[11px] text-slate-500">Revisa tu conexión o vuelve a escribir la búsqueda.</p>
            </div>
          ) : results.length === 0 ? (
            <div className="px-4 py-5 text-center">
              <UserPlus className="mx-auto h-5 w-5 text-slate-600" />
              <p className="mt-2 text-sm text-slate-400">No encontramos a esa persona.</p>
              <p className="mt-1 text-[11px] text-slate-600">Debe haber iniciado sesión en FrameFlow al menos una vez.</p>
            </div>
          ) : results.map((profile) => (
            <button key={profile.uid} type="button" role="option" aria-selected="false" onClick={() => onSelectedChange(profile)} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left transition hover:bg-cyan-400/10">
              ...
              <span className="rounded-full border border-slate-700 px-2 py-1 text-[9px] text-slate-500">Seleccionar</span>
            </button>
          ))}
        </div>
      )}
