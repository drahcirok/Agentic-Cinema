...
export default function ProfileMenu({
  ...
  async function saveProfile(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(body.detail ?? "No se pudo guardar tu perfil.");
      ...
      if (avatar) {
        ...
        if (!avatarResponse.ok)
          throw new Error(avatarBody.detail ?? "No se pudo subir la foto.");
  ...
  async function resetAvatar() {
    ...
    try {
      ...
      const body = await response.json();
      if (!response.ok) throw new Error(body.detail ?? "No se pudo restablecer la foto.");
      onProfileChange(body as UserProfile);
      setAvatar(null);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "No se pudo restablecer la foto.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="relative z-40">
      ...
      {open && (
        <>
          <button type="button" aria-label="Cerrar menú de perfil" className="fixed inset-0 z-[-1] cursor-default" onClick={() => setOpen(false)} />
          <div role="menu" aria-label="Opciones de perfil" className="absolute right-0 mt-3 w-72 overflow-hidden rounded-2xl border border-slate-700/80 bg-[#0d1828]/95 p-2 shadow-[0_24px_80px_rgba(0,0,0,.45)] backdrop-blur-xl landing-rise">
        ...
        <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-[#050a12]/85 p-4 py-6 backdrop-blur-sm">
          <form role="dialog" aria-modal="true" aria-labelledby="profile-dialog-title" onSubmit={saveProfile} className="workspace-modal max-h-[calc(100dvh-3rem)] w-full max-w-lg overflow-y-auto rounded-3xl border border-cyan-300/20 bg-[#0d1828] shadow-[0_30px_100px_rgba(0,0,0,.65)]">
            <div className="relative border-b border-white/5 bg-gradient-to-br from-cyan-400/10 via-transparent to-violet-400/10 px-6 py-6 sm:px-8">
              <button type="button" onClick={closeEditor} className="absolute right-5 top-5 rounded-full p-2 text-slate-500 transition hover:bg-white/5 hover:text-white" aria-label="Cerrar perfil">
                <X className="h-4 w-4" />
              </button>
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="flex items-center gap-2 text-[10px] font-bold tracking-[.22em] text-cyan-300"><Sparkles className="h-3.5 w-3.5" /> IDENTIDAD FRAMEFLOW</p>
                  <h2 id="profile-dialog-title" className="mt-2 text-2xl font-semibold text-white">Tu perfil</h2>
                  <p className="mt-1 text-sm text-slate-400">Así te reconocerá tu equipo.</p>
                </div>
              </div>
            </div>
            <div className="space-y-5 px-6 py-6 sm:px-8">
              <label className="block text-xs font-semibold uppercase tracking-[.12em] text-slate-400">
                Nombre visible
                <input required minLength={2} maxLength={80} value={displayName} onChange={(event) => setDisplayName(event.target.value)} className="workspace-input mt-2 w-full" placeholder="Tu nombre" />
              </label>
              <label className="block text-xs font-semibold uppercase tracking-[.12em] text-slate-400">
                Nombre de usuario
                <div className="workspace-input mt-2 flex items-center gap-1 focus-within:border-cyan-300/70">
                  <span className="text-cyan-300">@</span>
                  <input required minLength={3} maxLength={30} value={username} onChange={(event) => setUsername(event.target.value.toLowerCase().replace(/[^a-z0-9._-]/g, ""))} className="min-w-0 flex-1 bg-transparent text-white outline-none" placeholder="usuario" />
                </div>
                <span className="mt-2 block text-[11px] font-normal normal-case tracking-normal text-slate-500">Único en FrameFlow · letras, números, punto, guion o _</span>
              </label>
              <label className="block text-xs font-semibold uppercase tracking-[.12em] text-slate-400">
                Nueva foto <span className="font-normal normal-case text-slate-500">(opcional · JPG, PNG o WEBP · máx. 2 MB)</span>
              ...
              <div className="flex flex-wrap justify-between gap-3 border-t border-white/5 pt-5">
                <button type="button" onClick={() => void resetAvatar()} disabled={busy || !profile.has_custom_avatar} className="text-xs text-slate-500 transition hover:text-cyan-200 disabled:cursor-not-allowed disabled:opacity-40">Usar foto de Google</button>
                <div className="flex gap-2">
                  <button type="button" onClick={closeEditor} disabled={busy} className="action-button reject">Cancelar</button>
                  <button disabled={busy} className="flex min-h-10 items-center gap-2 rounded-xl bg-cyan-300 px-4 py-2 text-sm font-bold text-cyan-950 transition hover:-translate-y-0.5 hover:bg-cyan-200 disabled:opacity-50">
                    {busy && <LoaderCircle className="h-4 w-4 animate-spin" />} Guardar perfil
                  </button>
                </div>
              </div>
            </div>
